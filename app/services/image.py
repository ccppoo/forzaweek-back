...
def delete_uploade_image(originImageURL: str, user: UserAuth) -> Union[Url, None]:
    ...
    # should be
    match_user_id = head_resposne.Metadata.user_id == user.user_id
    match_user_oid = head_resposne.Metadata.user_doc_id == str(user.id)

    if match_user_id and match_user_oid:
